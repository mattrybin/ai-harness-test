...
- `app/src/test/tools.test.ts` — the tool tests. `node --test` against a temp dir, no Electron.
...
- `app/src/test/mcp.test.ts` — spawns `dist/modules/mcp.js` and talks JSON-RPC to it. `node --test`, no Electron.
